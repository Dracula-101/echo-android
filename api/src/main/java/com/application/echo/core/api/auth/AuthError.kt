...
sealed class AuthError(
    ...
    data class AccountLocked(
        override val message: String,
    ) : AuthError("AUTH_ACCOUNT_LOCKED", message)

    data class AccountDisabled(
        override val message: String,
    ) : AuthError("AUTH_ACCOUNT_DISABLED", message)

    data class PasswordExpired(
        override val message: String,
    ) : AuthError("AUTH_PASSWORD_EXPIRED", message)

    data class TwoFactorRequired(
        override val message: String,
    ) : AuthError("AUTH_2FA_REQUIRED", message)

    data class InvalidTwoFactorCode(
        override val message: String,
    ) : AuthError("AUTH_INVALID_2FA_CODE", message)

    data class EmailVerificationFailed(
        override val message: String,
    ) : AuthError("AUTH_EMAIL_VERIFY_FAILED", message)

    data class PhoneVerificationFailed(
        override val message: String,
    ) : AuthError("AUTH_PHONE_VERIFY_FAILED", message)

    // ── Token ────────────────────────────────────────────────────────

    data class InvalidToken(
        override val message: String,
    ) : AuthError("AUTH_INVALID_TOKEN", message)

    data class TokenGenerationFailed(
        override val message: String,
    ) : AuthError("AUTH_TOKEN_GEN_FAILED", message)

    data class TokenValidationFailed(
        override val message: String,
    ) : AuthError("AUTH_TOKEN_VALIDATE_FAILED", message)

    data class InvalidRefreshToken(
        override val message: String,
    ) : AuthError("AUTH_INVALID_REFRESH_TOKEN", message)

    data class RefreshTokenExpired(
        override val message: String,
    ) : AuthError("AUTH_REFRESH_TOKEN_EXPIRED", message)
    ...
    ) : AuthError("AUTH_EMAIL_EXISTS", message)

    data class InvalidEmail(
        override val message: String,
    ) : AuthError("AUTH_INVALID_EMAIL", message)

    data class InvalidPhone(
        override val message: String,
    ) : AuthError("AUTH_INVALID_PHONE", message)

    data class WeakPassword(
        override val message: String,
        val constraints: String? = null,
    ) : AuthError("AUTH_PASSWORD_WEAK", message)

    data class TermsNotAccepted(
        override val message: String,
    ) : AuthError("AUTH_TERMS_NOT_ACCEPTED", message)

    data class PasswordHashFailed(
        override val message: String,
    ) : AuthError("AUTH_PASSWORD_HASH_FAILED", message)

    // ── Session ──────────────────────────────────────────────────────

    data class SessionNotFound(
        override val message: String,
    ) : AuthError("AUTH_SESSION_NOT_FOUND", message)

    data class SessionExpired(
        override val message: String,
    ) : AuthError("AUTH_SESSION_EXPIRED", message)

    data class SessionCreateFailed(
        override val message: String,
    ) : AuthError("AUTH_SESSION_CREATE_FAILED", message)

    data class SessionUpdateFailed(
        override val message: String,
    ) : AuthError("AUTH_SESSION_UPDATE_FAILED", message)
    ...
    ) : AuthError("AUTH_TOO_MANY_FAILED_ATTEMPTS", message)

    data class SuspiciousActivity(
        override val message: String,
    ) : AuthError("AUTH_SUSPICIOUS_ACTIVITY", message)

    data class IpBlocked(
        override val message: String,
    ) : AuthError("AUTH_IP_BLOCKED", message)

    data class DeviceNotTrusted(
        override val message: String,
    ) : AuthError("AUTH_DEVICE_NOT_TRUSTED", message)

    // ── Database ─────────────────────────────────────────────────────

    data class DatabaseError(
        override val message: String,
    ) : AuthError("AUTH_DATABASE_ERROR", message)
    ...
    ) : AuthError("NETWORK_ERROR", message)
    ...
    companion object {
         ...
         * typed [AuthError]. Falls back to [Unknown] for unrecognized codes.
         */
        fun fromApiError(apiError: ApiError): AuthError = when (apiError.code) {
            // Authentication
         ...
         */
        fun fromNetworkException(exception: NetworkException): AuthError = when (exception) {
